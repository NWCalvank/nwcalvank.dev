...
excerpt: Only keep the data that sparks joy.
categories: [complete]
---
...
Python uses a different call signature, but the logic is the same as with the JavaScript dot-chaining example above. We start with our results, which is a list of every character.
